...
**状态：提案（2026-09），尚未开工。** 本文记录一次对市面 AI Coding 硬件的调研、米遥能低成本复刻的能力清单、分阶段计划，以及后续接入桌面小屏 / 开发板副屏的接口设计。所有阶段都不修改遥控器固件——小米 2 Pro 只开放 ATVV 语音和 HID 按键，米遥的硬件面固定为 **12 个按键 + 按住说话麦克风**，其余能力全部在 Mac 侧实现。
